...
function getArgsFromCommander() {
    program
        ...
        .option(`-w, --watch`, `Whether or not to continuously watch the target tables for new rows`, () => true, false)
        .option(`-m, --max-workers <count>`, `Limit the number of parallel workers used to sync tables (defaults to number of cores)`)
        .option(`-p, --setup <command>`, `Optionally specify a command that postgrab will run prior to connecting/syncing any tables`)
        .option(`-l, --local <uri|command>`, `Local database connection string or a command which returns a connection string or config object`)
        .option(`-r, --remote <uri|command>`, `Remote database connection string or a command which returns a connection string or config object`)
        .option(`-c, --config <path>`, `Path to postgrab config file (by default, postgrab will traverse upward until it finds '.postgrab.yaml')`)
