...
class PostgrabBaseClass {
    constructor(config) {
        ...
        lodash_1.default.forEach(
            [interfaces_1.DatabaseSource.LOCAL, interfaces_1.DatabaseSource.REMOTE],
            (source) => {
    ...
    createPool(source) {
        ...
        this.pool[source].on('error', (e) => {
            if (!/terminating connection/.test(e.message)) {
                console.error(
                    chalk_1.default.red(
                        `Error encountered in the ${source} query pool: ${e.message}`,
                    ),
                )
            }
        })
    ...
    abstractQuery(source, queryable) {
        ...
        return typeof queryable === 'string'
            ? result.then((val) => lodash_1.default.get(val, `rows`))
            : result
    }
    get query() {
        return {
    ...
    async getRemoteMetadataForTargetTables() {
        ...
        await Promise.all(
            lodash_1.default.map(tables, async (table) => {
      ...
      `,
                    ...
                    .then((rows) =>
                        lodash_1.default.map(rows, ({ name, definition }) => {
                            const whereClause = (definition.match(/WHERE (.*)/i) || [])[1]
                            const columns = (0, helpers_1.extractColumnNamesFromSqlStatement)(
                                definition,
                            )
    ...
    `,
        ...
        if (source === interfaces_1.DatabaseSource.LOCAL) return tableNames
    ...
    async streamingMergeCopy(tableData, remoteCmd) {
        ...
        const nonGeneratedColumns = tableData.columns
            ...
            .join(', ')
        const selectColumns = nonGeneratedColumns || '*'
        remoteCmd =
            remoteCmd ||
            `
            COPY (SELECT ${selectColumns} FROM ${SCHEMA_AND_TABLE}${remoteWhereClause ? ` ${remoteWhereClause}` : ''})
            TO STDOUT`
        let uniqueWhereClauseParts = lodash_1.default.flatMap(
            tableData.uniqueIndices,
            ({ columns, whereClause }) => {
                const hasJSONColumnAccessors = lodash_1.default.find(columns, (column) =>
                    lodash_1.default.includes(
                        constants_1.JSON_COLUMN_ACCESSORS,
                        lodash_1.default.get(column, '0.String.str'),
                    ),
                )
                if (hasJSONColumnAccessors) return
                const clauses = lodash_1.default.map(
                    columns,
                    (col) => col && `target_table."${col}" = temp_table."${col}"`,
                )
                if (whereClause) {
                    clauses.push(
                        ...lodash_1.default.map([`target_table`, `temp_table`], (prefix) => {
                            return (0, helpers_1.prefixColumnNamesInWhereClause)(
                                whereClause,
                                prefix,
                            )
                        }),
        ...
        uniqueWhereClauseParts = lodash_1.default.compact(uniqueWhereClauseParts)
        const localCmd = `
    ...
    async testConnections() {
        for (const source of [
            ...
            interfaces_1.DatabaseSource.REMOTE,
        ]) {
    ...
    async getLocalMaxForTable(table) {
        ...
        const bookmarkMetadata = lodash_1.default.find(
            lodash_1.default.get(this.tables, [table, `columns`]),
            ({ name }) => name === bookmark,
        )
    ...
    async getRemoteWhereClauseForTable(table) {
        const localMax = await this.getLocalMaxForTable(table).then((max) =>
            lodash_1.default.isNumber(max) ? max : max && `'${max}'`,
        )
        const bookmark = this.config.bookmarks[table]
