...
class InteractiveConfig extends base_1.default {
    ...
    generateQuestionsFromTables() {
        return lodash_1.default.map(lodash_1.default.sortBy(lodash_1.default.keys(this.tables)), tableName => {
            const bookmarkableColumns = lodash_1.default.filter(this.tables[tableName].columns, column => {
                return lodash_1.default.includes(lodash_1.default.values(interfaces_1.BookmarkableDataType), column.type);
            });
            return {
                name: tableName,
                type: `list`,
                message: `Choose a bookmark column for ${chalk_1.default.green(tableName)}`,
                choices: lodash_1.default.orderBy(bookmarkableColumns, ({ name, type }) => {
                    if (name === interfaces_1.PreferredColumnName.UPDATED_AT) {
                        ...
                        switch (type) {
                            case interfaces_1.BookmarkableDataType.TIMESTAMP_WITH_TIME_ZONE:
                                return 2;
                            case interfaces_1.BookmarkableDataType.TIMESTAMP_WITHOUT_TIME_ZONE:
                                return 2;
    ...
    persistChoices() {
        ...
        configFile[`tables`] = (0, helpers_1.objectOrderBy)(configTables, [`dump`, val => !!val.bookmark], [`desc`, `desc`]);
