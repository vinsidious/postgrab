...
class SyncTables extends base_1.default {
    ...
    async run() {
        ...
        if (this.config.truncate)
            await this.truncateLocalTargetTables();
        ...
        this.status.info(`Syncing ${(0, pluralize_1.default)('table', tableCount, true)} across ${(0, pluralize_1.default)('worker', this.workerCount, true)}`);
    ...
    async spawnWorkerForTable(worker, table) {
        ...
        let count = 0, remoteCount, localCount = 1, whereClause = ``;
        ...
        if (localCount >= remoteCount) {
            status(chalk_1.default.green(`✔ `) + `No new rows to dump for ${chalk_1.default.blue.underline(table)} table`);
            this.workers.push(worker);
        ...
        await new Promise((resolve) => {
            ...
            child.on(`error`, (error) => {
                console.log(chalk_1.default.red(`Error encountered: ${chalk_1.default.white(error.toString())}`));
                process.exit();
            });
            child.stderr.on(`data`, (data) => {
                console.log(chalk_1.default.red(`Error encountered: ${chalk_1.default.white(data.toString())}`));
            });
    ...
    async getCountForTableAndSource(table, source) {
        ...
        return await this.query[source](`${withStatement ? withStatement : ''}
      SELECT COUNT(*)
    ...
    async localAndRemoteCountsMatch(table) {
        const localCount = await this.getCountForTableAndSource(table, interfaces_1.DatabaseSource.LOCAL);
