...
class SyncSchema extends base_1.default {
    async run() {
        ...
        try {
            ...
            if (missingExtensions.length) {
                this.status.start(`Installing missing extensions: ${chalk_1.default.green(missingExtensions.join(chalk_1.default.white(`, `)))}`);
                const installExtensionsCmds = lodash_1.default.map(missingExtensions, ext => `CREATE EXTENSION IF NOT EXISTS "${ext}";`).join(` `);
        ...
        try {
            ...
            const localStream = (0, child_process_1.spawn)(`pg_restore`, [`-Oxs`, `-d`, this.config.local]);
    ...
    async dropNecessaryTables(count = 1) {
        if (this.config.tables.length === this.remoteTableNames.length) {
            ...
            const dropCommands = lodash_1.default.map(this.config.tables, table => `DROP TABLE IF EXISTS "${table}" CASCADE;`).join(` `);
    ...
    async terminateOpenConnections() {
        ...
        return await new Promise(resolve => {
            (0, helpers_1.bufferToStream)(new Buffer(`
