...
class App {
    ...
    async mergeConfigFileWithUserArgs(configFile, userArgs) {
        ...
        let tables = [];
        if (lodash_1.default.size(userArgs.tables))
            tables.push(...userArgs.tables);
        ...
        const watchIntervalSeconds = configFile.watch_interval_seconds || defaults.WATCH_INTERVAL_SECONDS;
        ...
        const exclude = lodash_1.default.size(userArgs.exclude)
            ? userArgs.exclude
            : lodash_1.default.keys(lodash_1.default.pickBy(configFile.tables, (table) => table && table.dump === false));
    ...
    convertPartialsIntoCTE(partials, schema) {
        ...
        const unorderedDependencyMap = lodash_1.default.map(partials, (partial, table) => {
            const dependencies = lodash_1.default.map(partial.match(constants.PARTIAL_REF_REGEX), (partial) => partial.replace(/\W/g, ''));
        ...
        while (lodash_1.default.size(unorderedDependencyMap)) {
            ...
            const nextItem = lodash_1.default.find(unorderedDependencyMap, ({ table, dependencies }) => lodash_1.default.isEmpty(dependencies) ||
                lodash_1.default.isEmpty(lodash_1.default.difference(dependencies, alreadyOrderedTables)));
            if (!nextItem) {
            ...
            const expandedDependencies = (function recurseDependencies({ table, dependencies } = {}, deps = []) {
                if (!table)
                    return;
                deps.push(table);
                lodash_1.default.forEach(dependencies, (dep) => recurseDependencies(lodash_1.default.find(orderedDependencyMap, { table: dep }), deps));
                return lodash_1.default.uniq(deps);
            })(nextItem);
            orderedDependencyMap.push({
                table: nextItem.table,
                dependencies: lodash_1.default.filter(alreadyOrderedTables, (table) => lodash_1.default.includes(expandedDependencies, table)),
            });
        }
        ...
        lodash_1.default.forEach(orderedDependencyMap, ({ table, dependencies }) => {
            if (lodash_1.default.isEmpty(dependencies))
                return (withStatements[table] = '');
            const withStatement = lodash_1.default.map(dependencies, (table) => `${table} AS (SELECT * FROM "${schema}"."${table}" ${partials[table]})`).join(',\n');
    ...
    async parseDbParams(params) {
        ...
        try {
            ...
            if ((0, helpers_1.isPgUri)(paramsResult))
                return paramsResult;
            try {
    ...
    findConfigFile(filepath) {
        ...
        if (!filepath) {
            ...
            console.error(`\nAdd your connection details to '.postgrab.yaml' and then run ${chalk_1.default.green(`postgrab --init`)} ` + `to begin the interactive configuration process\n`);
            process.exit();
        }
        return filepath;
    }
