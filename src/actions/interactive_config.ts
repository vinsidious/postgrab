...
export default class InteractiveConfig extends PostgrabBaseClass {
  ...
  private persistChoices() {
    ...
    const configTables = _.mapValues(this.answers, bookmark => {
      ...
      else if (bookmark !== LabelForInteractiveConfigDefaultOption.NO_COLUMN) config.bookmark = bookmark;
      return config;
    });
    configFile[`tables`] = objectOrderBy(configTables, [`dump`, val => !!val.bookmark], [`desc`, `desc`]);
